...
begin
	...
	md"Loading modules for CompareTmTs project ..."
end
...
begin
	...
	for inc = 1 : 10
		...
		ids   = NCDataset(joinpath(fol,"RCE_ExploreWTGSpace-P1282km300V64-member$(istr).nc"))
		p[:] += ids["p"][:]
...
function hump(radius,fine)
	...
	for ix = 1 : nx
		if abs(x[ix]) < radius
			y[ix] = cos(x[ix] / radius * pi/2) * 0.95
		end
	end
	return x,y
end
...
begin
	...
	axs[3].plot(sin.(((1.9:0.01:3).-1.9)*pi/1.1)/1.2,1.9:0.01:3,c="red7")
	axs[3].scatter(sin.(((2:0.05:3).-2)*pi)/5,2:0.05:3,c="red3",s=2)
	axs[3].arrow(0.23,2.45,0.6,0,width=0.02,head_width=0.05,head_length=0.15,edgecolor="none",facecolor="gray4",length_includes_head=true)
	axs[3].format(grid=false,xlabel=L"$\delta T_v$")
	
	axs[4].fill_between([-2,0],2.1,1.85,color="gray",alpha=0.1)
	axs[4].plot(-sin.(((2.05:0.01:3).-2.05)*pi/0.95)/1.2,2.05:0.01:3,c="blue7")
	axs[4].scatter(-sin.(((2:0.05:3).-2)*pi)/5,2:0.05:3,c="blue3",s=2)
	axs[4].arrow(-0.23,2.525,-0.6,0,width=0.02,head_width=0.05,head_length=0.15,edgecolor="none",facecolor="gray4",length_includes_head=true)
	...
	x0d8,y0d8 = hump(0.8,true); y0d8 = y0d8 * 0.4
	x0d1,y0d1 = hump(0.2,false);
	c = axs[6].scatter(x0d1,y0d1,c=y0d1,cmap="RdBu_r",s=2,levels=-1.2:0.001:1.2)
	axs[6].scatter(x0d8,y0d8,c=y0d8,cmap="RdBu_r",s=2,levels=-1.2:0.001:1.2)
	axs[6].arrow(0,0.05,0,0.85,width=0.02,head_width=0.05,head_length=0.3,edgecolor="none",facecolor="red8",length_includes_head=true)
	axs[6].arrow(0.06,0.75,0,-0.3,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[6].arrow(-0.06,0.75,0,-0.3,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[6].arrow(0.25,0.05,0,0.2,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[6].arrow(-0.25,0.05,0,0.2,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[6].arrow(0.4,0.05,0,0.1,width=0.01,head_width=0.05,head_length=0.1,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[6].arrow(-0.4,0.05,0,0.1,width=0.01,head_width=0.05,head_length=0.1,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[6].format(ylabel=L"$\delta T_v$",ltitle="(ii)")
	
	axs[7].scatter(x0d1,-y0d1,c=-y0d1,cmap="RdBu_r",s=2,levels=-1.2:0.001:1.2)
	axs[7].scatter(x0d8,-y0d8,c=-y0d8,cmap="RdBu_r",s=2,levels=-1.2:0.001:1.2)
	axs[7].arrow(0,-0.05,0,-0.8,width=0.02,head_width=0.05,head_length=0.3,edgecolor="none",facecolor="blue8",length_includes_head=true)
	axs[7].arrow(0.06,-0.75,0,0.3,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[7].arrow(-0.06,-0.75,0,0.3,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="red2",length_includes_head=true)
	axs[7].arrow(0.25,-0.05,0,-0.2,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[7].arrow(-0.25,-0.05,0,-0.2,width=0.01,head_width=0.05,head_length=0.2,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[7].arrow(0.4,-0.05,0,-0.1,width=0.01,head_width=0.05,head_length=0.1,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[7].arrow(-0.4,-0.05,0,-0.1,width=0.01,head_width=0.05,head_length=0.1,edgecolor="none",facecolor="blue2",length_includes_head=true)
	axs[7].format(ltitle="(ii)")
	axs[7].colorbar(c,label=L"$\delta T_v$",ticklabels=["-","0","+"],locator=-1:1)
	
	for ii in 6 : 7
		    axs[ii].arrow(0.2,0.6,0.6,0,width=0.1,head_width=0.25,head_length=0.05,edgecolor="none",facecolor="gray4",length_includes_head=true)
		    axs[ii].arrow(0.2,-0.6,0.6,0,width=0.1,head_width=0.25,head_length=0.05,edgecolor="none",facecolor="gray4",length_includes_head=true)
		    axs[ii].arrow(-0.2,0.6,-0.6,0,width=0.1,head_width=0.25,head_length=0.05,edgecolor="none",facecolor="gray4",length_includes_head=true)
		    axs[ii].arrow(-0.2,-0.6,-0.6,0,width=0.1,head_width=0.25,head_length=0.05,edgecolor="none",facecolor="gray4",length_includes_head=true)
		axs[ii].text(-0.95,0.55,"+")
		axs[ii].text(-0.95,-.75,L"-")
		axs[ii].text(0.9,0.55,"+")
